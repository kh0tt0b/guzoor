  ...
  return (
    <article className="card card-hover group flex h-full flex-col overflow-hidden">
      <Link
        ...
        aria-label={`Read the story of ${hero.name}`}
      >
        <LazyImage
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <p className="mt-0.5 text-sm font-medium text-accent">{hero.title}</p>
        ...
        <div className="mt-4 flex items-center justify-between border-t border-accent/10 pt-4 dark:border-primary-500/40">
          <div className="flex items-center gap-1.5 text-xs text-accent-400">
            ...
            <span className="line-clamp-1 italic">{hero.quote}</span>
          </div>
          <Link
            to={`/heroes/${hero.id}`}
